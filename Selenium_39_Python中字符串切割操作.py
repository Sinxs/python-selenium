...
class GetSubString(object):
    ...
    def get_search_result(self):
        ...
        #print(driver.find_element_by_xpath("//*/span[@class='nums_text']").text)
        search_result_string1 = driver.find_element_by_xpath("//*/span[@class='nums_text']").text


        #print(search_result_string)
        print(search_result_string1)
        new_string = search_result_string.split("约")[1]
        last_result = new_string.split("个")[0]
        # new_string = search_result_string1.split('约')[1]  # 第一次切割得到 xxxx个，[1]代表切割右边部分
        print(new_string)
